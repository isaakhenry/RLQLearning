...
class NeuralNetwork:
    ...
    def __init__(self, num_actions, replay_memory):
        ...
        init = tf.compat.v1.truncated_normal_initializer(mean=0.0, stddev=2e-2)
        ...
        net = tf.compat.v1.layers.conv2d(inputs=net, name='layer_conv1',
                               filters=16, kernel_size=3, strides=2,
                               padding=padding,
                               kernel_initializer=init, activation=activation)

        # Second convolutional layer.
        net = tf.compat.v1.layers.conv2d(inputs=net, name='layer_conv2',
                               filters=32, kernel_size=3, strides=2,
                               padding=padding,
                               kernel_initializer=init, activation=activation)

        # Third convolutional layer.
        net = tf.compat.v1.layers.conv2d(inputs=net, name='layer_conv3',
        ...
        net = tf.compat.v1.layers.dense(inputs=net, name='layer_fc1', units=1024,
                              kernel_initializer=init, activation=activation)

        # Second fully-connected layer.
        net = tf.compat.v1.layers.dense(inputs=net, name='layer_fc2', units=1024,
                              kernel_initializer=init, activation=activation)

        # Third fully-connected layer.
        net = tf.compat.v1.layers.dense(inputs=net, name='layer_fc3', units=1024,
                              kernel_initializer=init, activation=activation)

        # Fourth fully-connected layer.
        net = tf.compat.v1.layers.dense(inputs=net, name='layer_fc4', units=1024,
                              kernel_initializer=init, activation=activation)

        # Final fully-connected layer.
        net = tf.compat.v1.layers.dense(inputs=net, name='layer_fc_out', units=num_actions,
    ...
    def get_layer_tensor(self, layer_name):
        ...
        tensor = tf.compat.v1.get_default_graph().get_tensor_by_name(tensor_name)
